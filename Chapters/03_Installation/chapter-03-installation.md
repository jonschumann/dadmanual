...
The **DADDriverSetup** application controls the sample rate, channel count, and device selection for the Thunderbolt 3 audio driver. Changes made here are immediately reflected in **Audio MIDI Setup** and are visible to any Core Audio–compatible application on the computer.
...
| **Sample Rate** | 44.1, 48, 88.2, 96, 176.4, 192, 352.8, 384 kHz | Must match your DAW project and hardware configuration |
| **Channels** | 16, 64, 128, 256 (bi-directional) | All Thunder\|Core interfaces support 256 bi-directional channels in hardware; reduce this number to lower CPU overhead if your workflow requires fewer channels |
...
| **Channels** | 16, 64, 128, 256 (bi-directional) | Reduce from maximum if not all channels are needed to improve system performance |
