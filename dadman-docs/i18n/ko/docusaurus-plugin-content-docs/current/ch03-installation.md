...
> **NOTICE:** The Thunderbolt 3 driver provides the audio _transport_ between your computer and the hardware — your DAW sees it as an audio interface. DADman provides the _control_ layer — routing, gain, monitoring, and configuration. Both can be installed and run on the same computer simultaneously without conflict.
...
_[Placeholder: figures to be inserted — TB3 driver installation screenshots (macOS and Windows), DADDriverSetup configuration window, Audio MIDI Setup showing the DAD interface, macOS Privacy & Security / Accessories screen, Windows ASIO dialogue]_

_[Pending from DAD engineering: full firmware update procedure cross-reference, confirmed IP addressing instructions per hardware model]_
